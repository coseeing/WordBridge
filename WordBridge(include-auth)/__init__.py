...
#module_names = ["cryptography.hazmat.primitives", "cryptography.hazmat.primitives.kdf", "cryptography.utils"]
#for module_name in module_names:
#	if module_name in sys.modules:
#		del sys.modules[module_name]
#	sys.modules[module_name] = importlib.import_module(f"package.{module_name}")
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def terminate(self, *args, **kwargs):
		...
		# Ensure any pending auth flow is aborted and singleton is cleared
		try:
			shutdown_oidc()
		except Exception:
			pass
	...
	def correctTypo(self, request):
		...
		if corrector_config['model']['llm_access_method'] != "coseeing_relay":
			...
			try:
				oidc = get_oidc()
				access_token = oidc.get_valid_access_token_async(auto_login=True).result()
			except Exception as e:
				access_token = ""
				log.warning(_("Sorry, an error occurred while get valid access_token, the details are: {e}").format(e=e))
			...
			headers = {
				...
				"X-Issuer": GOOGLE["issuer"],
			}
			try:
				response = requests.post(f"{COSEEING_BASE_URL}/proofreader", headers=headers, json=data, timeout=120)
			...
			if response.status_code == 401:
				...
				try:
					if data.json()["detail"]["error"] == "registration_required":
						endpoint = data.json()["detail"]["endpoint"]
						data = {}
						response = requests.post(f"{COSEEING_BASE_URL}{endpoint}", json=data, timeout=120)
				except Exception:
					return
		...
		diff = strings_diff(request, data)
	...
	@script(
		...
		description=_("login Coseeing"),
		category=ADDON_SUMMARY,
	)
	def script_login_coseeing(self, gesture):
		oidc = get_oidc()
		oidc.login_async()
		return
		...
		def _done(f: Future):
			try:
				...
				wx.CallAfter(print, f"[login] error: {e}\n")
			except Exception as e:
				wx.CallAfter(print, f"[login] error: {e}\n")
			else:
				wx.CallAfter(print, f"[login] success:\n{json.dumps(payload, ensure_ascii=False, indent=2)}\n")
